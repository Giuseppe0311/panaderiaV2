...
export class SucursalesComponent {
  ...
  loadData() {
    this.apiserviceadmin
      .getData(ApiType.Public, 'sucursales', { idempresa: this.idempresa })
      .subscribe({
        next: (data) => {
          this.data = data;
  ...
  registrar() {
    ...
    if (this.formulario.valid && this.idempresa !== null) {
      ...
      this.apiserviceadmin
        .postData(ApiType.Admin, 'sucursales', data)
        .subscribe(
          (res: any) => {
            console.log(res);
            this.isLoading = false;
  ...
  actualizar() {
    ...
    if (this.updateform.valid && this.idempresa !== null) {
      ...
      const data = {
        ...
        informacion: this.updateform.value.informacion,
        idempresa: this.idempresa,
        id: this.updateform.value.id,
      };
      this.apiserviceadmin
        .updateData(ApiType.Admin, 'sucursales', this.updateform.value.id, data)
        .subscribe(
          (res: any) => {
            console.log(res);
            this.loadData();
  ...
  eliminar() {
    if (this.idAEliminar !== null) {
      this.isLoading = true;
      this.apiserviceadmin
        .deleteData(ApiType.Admin, 'sucursales', this.idAEliminar)
        .subscribe(
          (res: any) => {
            console.log(res);
            this.loadData();
          ...
          (err: any) => {
            this.isLoading = false;
            alert('Error al eliminar la sucursal' + err);
          }
        );
    } else {
      alert(
        'No se puede eliminar  la sucursal hubo un error al obtener el id del producto'
      );
    }
  }
}
