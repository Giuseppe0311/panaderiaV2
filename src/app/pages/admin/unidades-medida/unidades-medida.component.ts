...
export class UnidadesMedidaComponent {
  ...
  loadData() {
    this.apiserviceadmin
      .getData(ApiType.Public, 'unidadesmedida', { idempresa: this.idempresa })
      .subscribe({
        next: (data) => {
          this.data = data;
        },
  ...
  registrar() {
    ...
    if (this.formulario.valid && this.idempresa !== null) {
      ...
      console.log(data);
      this.apiserviceadmin
        .postData(ApiType.Admin, 'unidadesmedida', data)
        .subscribe(
          (res: any) => {
            console.log(res);
            this.isLoading = false;
  ...
  eliminar() {
    if (this.idAEliminar !== null) {
      this.isLoading = true;
      this.apiserviceadmin
        .deleteData(ApiType.Admin, 'unidadesmedida', this.idAEliminar)
        .subscribe(
          (res: any) => {
            console.log(res);
            this.loadData();
      ...
      alert(
        'No se puede eliminar la unidad de medida hubo un error al obtener el id del producto'
      );
    }
  }
}
